<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="listorders.aspx.cs" Inherits="QuanLySach.listorders" %>
...
<!-------------------------------------------------------------------------------------------------------> 
<asp:GridView ID="gridOrders" runat="server" AutoGenerateColumns="false" CssClass="table table-bordered" Width="100%" OnRowDataBound="gridOrders_RowDataBound">
    <Columns>
        <asp:BoundField DataField="id" HeaderText="Mã đơn hàng" />
        <asp:BoundField DataField="lastname" HeaderText="Tên khách hàng" />
        <asp:BoundField DataField="created_at" HeaderText="Ngày đặt" DataFormatString="{0:dd/MM/yyyy}" />
        <asp:TemplateField HeaderText="Trạng thái">
            <ItemTemplate>
                <span class='<%# GetStatusClass((string)Eval("status")) %>'><%# Eval("status") %></span>
            </ItemTemplate>
        </asp:TemplateField>
        <asp:TemplateField HeaderText="Thao tác">
            <ItemTemplate>
                <a class="btn btn-warning1" href='<%# "vieworders.aspx?id=" + Eval("id") %>'>Xem</a>
                <a class="btn btn-warning2" href='<%# "?action=delete&id=" + Eval("id") %>'>Hủy</a>
            </ItemTemplate>
        </asp:TemplateField>
    </Columns>
</asp:GridView>
